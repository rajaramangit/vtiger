	...
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			{if $BLOCK_FIELDS|@count lte 0}{continue}{/if}
                        <!--Modified By Nanda to hide recurring invoice  ifno-->
                        {if vtranslate($BLOCK_LABEL, $MODULE) eq "Recurring Invoice Information"}
			<table class="table table-bordered blockContainer showInlineTable" style='display:none;'>
                        {else}
			...
			{if ($BLOCK_LABEL eq 'LBL_ADDRESS_INFORMATION') and ($MODULE neq 'PurchaseOrder') }
				...
				<div style='display:none;'>	<label class="muted pull-right marginRight10px" name="togglingHeader">{vtranslate('LBL_BILLING_ADDRESS_FROM', $MODULE)}</label></div>
				</td>
				...
				<div style='display:none;'>	<label class="muted pull-right marginRight10px" name="togglingHeader">{vtranslate('LBL_SHIPPING_ADDRESS_FROM', $MODULE)}</label></div>
				</td>
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{/if}
                           ...
                           {if $FIELD_MODEL->get('name') eq "assigned_user_id" or $FIELD_MODEL->get('name') eq "sostatus"}
                             <td class="fieldLabel {$WIDTHTYPE}" style='display:none;'>
                           {else}
                            <td class="fieldLabel {$WIDTHTYPE}">
                           {/if}
			   <!--<input type="text" name="modulename" value="{$FIELD_MODEL->get('name')}-{$FIELD_MODEL->get('uitype')}-{$FIELD_MODEL->get('fieldvalue')}">-->
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
                                         {if $FIELD_MODEL->get('name') eq "assigned_user_id" or $FIELD_MODEL->get('name') eq "sostatus"}
                                    <td style='display:none;' class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if} {if $FIELD_MODEL->get('uitype') eq '20'} colspan="3"{/if}>
                                {else}
