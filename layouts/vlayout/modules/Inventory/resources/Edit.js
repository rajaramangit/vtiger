...
Vtiger_Edit_Js("Inventory_Edit_Js",{
    ...
    mapResultsToFields: function(referenceModule,element,responseData){
		...
		var new_row_id = parentRow.attr('id').split('row')[1];
		...
		for(var id in responseData){
			...
			var popup_qty = recordData.popup_qty;
			var popup_cmnt = recordData.popup_cmnt;
			if(popup_cmnt){
				if ( $( "#popup_cmnt_hidden_"+new_row_id ).length ) {
					$( "#popup_cmnt_hidden_"+new_row_id ).val(popup_cmnt);
				}else{
					lineItemNameElment.append("<input type='hidden' id='popup_cmnt_hidden_"+new_row_id+"' value='"+popup_cmnt+"' /\>");
				}				
			}else{
				if ( $( "#popup_cmnt_hidden_"+new_row_id ).length ) {
					$( "#popup_cmnt_hidden_"+new_row_id ).val('');
				}				
			}
	...
	lineItemPopupEventHandler : function(popupImageElement) {
		...
		var lineItemRow  = popupImageElement.closest('tr.lineItemRow');
		var lineItemProductOrServiceElement = lineItemRow.find('input.productName').closest('td');
		var sel_prod_id = jQuery('input.selectedModuleId',lineItemProductOrServiceElement).val();		
		var tr_id  = popupImageElement.closest('tr.lineItemRow').attr('id').split('row')[1];
		var sel_qty_val = jQuery('#qty'+tr_id).val();
		var sel_qty_opt = jQuery('#popup_cmnt_hidden_'+tr_id).val();
		jQuery('#sel_qty_val').val(sel_qty_val);
		jQuery('#sel_prod_id').val(sel_prod_id);
		jQuery('#sel_prod_opt').val(sel_qty_opt);
	...
	calculateNetTotal : function() {
		...
		if(netTotalValue < 300){
                 jQuery("#shipping_handling_charge").val(20);
                }
                else{
                 jQuery("#shipping_handling_charge").val(0); 
                }
	},
	...
	registerFinalDiscountShowEvent : function(){
        ...
        getRewardPointsEvent : function(){
                var thisInstance = this;
                jQuery('#getredeem').on('click',function(e){
                        var xmlhttp;
                        ...
                        var contactId = document.getElementsByName('account_id')[0].value;
                        var customerId=document.getElementById('account_id_display').value.split("#")[1].split(")")[0];
                        xmlhttp.open("GET","index.php?module=SalesOrder&action=GetRewardPoints&customerId="+customerId+"&contactId="+contactId,true);
                        xmlhttp.onreadystatechange=function(){
                        if(xmlhttp.readyState==4 && xmlhttp.status==200){
                            jQuery('#loadingListViewModal').modal('hide');
                            var response=JSON.parse(xmlhttp.responseText);               
                            if(!response.success)
                            {
                                response['minPoint']=0;
                                response['pointAvailable']=0;
                            }
                            jQuery('#redeempoint').val(response['pointAvailable']);
                            jQuery('#spendpoint').val(response['minPoint']);
                          //  document.getElementsByName('discount_final')[2].checked=true;
                          //  jQuery('#discount_amount_final').val( response['minPoint']);
                            document.getElementsByName('adjustmentType')[0].checked=true;
                            jQuery("#adjustment").val(response['minPoint']);
