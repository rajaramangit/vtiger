...
{strip}
        ...
        <div>
            {if $PRODUCT_ACTIVE eq 'true' && $SERVICE_ACTIVE eq 'true'}
                <div class="btn-toolbar">
                    ...
                    <span class="btn-group">
                        <button type="button" class="btn addButton" id="addService" style="display:none;">
    ...
    <table class="table table-bordered blockContainer lineItemTable" id="lineItemResult">
        <tr>
            <td  width="83%">
                <span style="width:20% !important;"><b><a class='redeembox' id='getredeem' style='cursor:pointer;color:green;font-wieght:bold'>&nbsp;Click here to get redeem points</a></b></span>&nbsp;<div class="pull-right"><strong>{vtranslate('LBL_ITEMS_TOTAL',$MODULE)}</strong></div>
            </td>
            <td>
        ...
        <tr>
            <td width="83%">
                ...
                <span style="width:20% !important;"><b>Maximun points to be spend</b></span>&nbsp;
                <span><input type='text' style="width:40px !important;" class='spendbox' id='spendpoint' readonly></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <span style="width:20% !important;"><b>Points avaliable</b></span>&nbsp;
                <span><input type='text' style="width:40px !important;" class='reedemtext' id='redeempoint' readonly></span>
                <span class="pull-right"><div style='display:none;'>(-)&nbsp;<b><a href="javascript:void(0)"  id="finalDiscount">{vtranslate('LBL_DISCOUNT',$MODULE)}</a></b></div></span>
            </td>
            <td>
               <div style='display:none;'><span id="discountTotal_final" class="pull-right discountTotal_final">{if $FINAL.discountTotal_final}{$FINAL.discountTotal_final}{else}0.00{/if}</span></div>
...
<!--                        <input type="radio" name="adjustmentType" option value="-" {if $FINAL.adjustment lt 0}checked{/if}>{vtranslate('LBL_DEDUCT',$MODULE)}-->
                        <input type="radio" name="adjustmentType" option value="-" checked>{vtranslate('LBL_DEDUCT',$MODULE)}
                    </div>
                    <div class="radio pull-right" style='display:none;'>
                        <!--<input type="radio" name="adjustmentType" option value="+"  {if $FINAL.adjustment gte 0}checked{/if}>-->
                        <input type="radio" name="adjustmentType" option value="+">{vtranslate('LBL_ADD',$MODULE)}&nbsp;&nbsp;
                    </div>
                </div>
            </td>
            <td>
                <span class="pull-right"><input id="adjustment" name="adjustment" type="text" data-validation-engine="validate[funcCall[Vtiger_PositiveNumber_Validator_Js.invokeValidation]]" class="lineItemInputBox" value="{if $FINAL.adjustment lt 0}{abs($FINAL.adjustment)}{elseif $FINAL.adjustment}{$FINAL.adjustment}{else}0.00{/if}" readonly></span>
            </td>
        </tr>
        <tr valign="top">
            <td  width="83%">
...
document.getElementById("currency_id_chzn").style.display='none';
document.getElementById("SalesOrder_editView_fieldName_subject").value='Sales by Call Center';
document.getElementById("SalesOrder_editView_fieldName_subject").readOnly=true;
}
{/literal}
</script>
