...
<div style='padding:4%;padding-top: 0;margin-bottom: 2%'>
	...
	<div class="row-fluid" style="padding:5px;padding-bottom:10px;">
		{$callhistory_sort=$MINILIST_WIDGET_MODEL->getTargetModule()}
                {if $callhistory_sort eq 'CallHistory'}
                <a class="pull-right" href="index.php?module={$MINILIST_WIDGET_MODEL->getTargetModule()}&view=List&mode=showListViewRecords&viewname={$WIDGET->get('filterid')}&orderby=calltime&sortorder=DESC">{vtranslate('LBL_MORE')}</a>
                {else}
